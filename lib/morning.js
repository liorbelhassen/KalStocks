...
const logoImg = (symbol) => {
  const u = logoUrl(symbol, 64)
  return u
    ? `<img src="${u}" width="18" height="18" alt="" style="border-radius:4px;background:#fff;vertical-align:middle;margin-left:6px;">`
    : ''
}

export async function assessOpen({ nameHe, symbol, date, isIndex }, apiKey, model = DEFAULT_MODEL) {
  const subject = isIndex ? `מדד "${nameHe}"` : `המניה "${nameHe}" (${symbol})`
  const prompt = `זהו בוקר ${date}, לפני פתיחת המסחר בבורסת תל אביב. אתה נותן סקירה מקדימה על ${subject}.
חקור מה קרה מאז סגירת המסחר הקודמת ובמהלך הלילה: סגירת שוקי ארה"ב, מסחר באסיה ובאירופה, מאקרו,
גאופוליטיקה, חדשות על החברה ועל הענף — כל מה שעשוי להשפיע על אופן הפתיחה של הנייר היום.
תן הערכה בעברית פשוטה ועממית (2-3 משפטים), שווה לכל נפש, כיצד צפוי הנייר להיפתח (גבוה יותר / נמוך יותר / ללא שינוי מהותי) ולמה.
...
export function buildMorningHtml({ dateStr, items = [] }) {
    ...
    ? items
      ...
      <div dir="rtl" style="background:#f6f8fa;border:1px solid #e6e9ee;border-radius:10px;padding:14px 16px;margin:10px 0;text-align:right;">
        <div style="display:flex;justify-content:space-between;align-items:baseline;">
          <span style="font-size:16px;font-weight:700;">${logoImg(it.symbol)}${it.nameHe}</span>
  ...
  <body style="margin:0;background:#ffffff;font-family:Arial,Helvetica,sans-serif;color:#24292f;">
    <div dir="rtl" style="max-width:600px;margin:0 auto;padding:24px 20px;text-align:right;direction:rtl;">
      <h1 style="font-size:22px;margin:0 0 2px;">☀️ סקירת בוקר — KalStocks</h1>
      <div style="color:#6a737d;font-size:13px;margin-bottom:6px;">${dateStr}</div>
      <div style="color:#6a737d;font-size:13px;margin-bottom:18px;">הערכה כיצד צפוי להיפתח המסחר בניירות שלך, על סמך חדשות הלילה.</div>
      ${cards}
