...
קריטי — דיוק: חפש את האירוע/החדשה הדומיננטית שמניעה את השוק היום, כולל מצב ביטחוני וגאופוליטי (הסלמה, מתקפות, מלחמה, מתיחות אזורית) — שהוא לרוב הגורם המרכזי לתנועות רוחביות בבורסת תל אביב. אם עלה אירוע ספציפי, ציין אותו בשמו (מי/מה/היכן), לא רק "מתיחות". בסס את ההסבר אך ורק על חדשות אמת שמצאת בחיפוש; אל תמציא סיבות פיננסיות (כמו "העלאת ריבית", "דוחות מאכזבים") שלא אימתת בפועל. אם כל השוק זז יחד (תנועה רוחבית) — הסבר את הסיבה המערכתית/הגאופוליטית האמיתית, לא סיבה ספציפית לחברה.
...
הערכה: <ההערכה בעברית, 2-3 משפטים מהותיים>`
  ...
  assessment = await verifyInsight(assessment, subject, keys, provider)
