...
export async function assessOpen({ nameHe, symbol, date, isIndex, session = 'morning' }, keys) {
...
כתוב בעברית בלבד — בלי אנגלית, בלי קישורים, בלי markdown, בלי כותרות. זו הערכה על סמך חדשות, לא ייעוץ השקעות.
...
הערכה: <ההערכה בעברית, 2-3 משפטים מהותיים>`
  ...
  const { text, sources, provider } = await askWithSearch(prompt, keys, { temperature: 0.4 })
  ...
  const assessment = cleanInsight(text.match(/הערכה:\s*([\s\S]+)/)?.[1] || text)
