...
export function buildDigestHtml({ dateStr, timeLabel, movers = [], all = [] }) {
    ...
    ? movers
      ...
      <div dir="rtl" style="background:#f6f8fa;border:1px solid #e6e9ee;border-radius:10px;padding:14px 16px;margin:10px 0;text-align:right;">
        <div style="font-size:16px;font-weight:700;">
          ${logoImg(m.symbol)}${m.nameHe} <span style="color:${color};direction:ltr;">${pctText(m.changePct)}</span>
        </div>
  ...
  const rows = all
    ...
    .map((s) => {
      ...
      return `
      <tr>
        <td style="padding:6px 10px;border-bottom:1px solid #eee;text-align:right;">${logoImg(s.symbol, 16)}${s.nameHe}</td>
