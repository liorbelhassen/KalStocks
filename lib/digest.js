...
export function buildDigestHtml({ dateStr, timeLabel, movers = [], all = [], logoToken }) {
    ...
    ? movers
      ...
      <div dir="rtl" style="background:#f6f8fa;border:1px solid #e6e9ee;border-radius:10px;padding:14px 16px;margin:10px 0;text-align:right;">
        <div style="font-size:16px;font-weight:700;">
          ${badgeHtml(m.symbol, m.nameHe, { kind: m.kind }, logoToken)}${m.nameHe} <span style="color:${color};direction:ltr;">${pctText(m.changePct)}</span>
        </div>
  ...
  const rows = all
    ...
    .map((s) => {
      ...
      return `
      <tr>
        <td style="padding:6px 10px;border-bottom:1px solid #eee;text-align:right;">${badgeHtml(s.symbol, s.nameHe, { kind: s.kind, isIndex: s.isIndex }, logoToken, 16)}${s.nameHe}</td>
  ...
  <body style="margin:0;background:#ffffff;font-family:Arial,Helvetica,sans-serif;color:#24292f;">
    <div dir="rtl" style="max-width:600px;margin:0 auto;padding:24px 20px;text-align:right;direction:rtl;">
      <h1 style="font-size:22px;margin:0 0 2px;">KalStocks — סיכום ${timeLabel || ''}</h1>
