...
// A "weak" answer = the model gave up or gave empty filler. Triggers one forceful retry.
export function isWeak(text) {
  const t = (text || '').trim()
  return t.length < 45 || /לא נמצא|אין מידע|אין נתונ|אין חדש|קשה להערי|מומלץ לעקוב|תלוי בשוק/.test(t)
}
