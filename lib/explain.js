...
import { askWithSearch, cleanInsight, isWeak, verifyInsight } from './llm.js'
...
קריטי — דיוק: חפש את האירוע/החדשה הדומיננטית שמניעה את השוק, כולל מצב ביטחוני וגאופוליטי (הסלמה, מתקפות, מלחמה, מתיחות אזורית) — שהוא לרוב הגורם המרכזי לתנועות רוחביות בבורסת תל אביב. אם עלה אירוע ספציפי, ציין אותו בשמו (מי/מה/היכן), לא רק "מתיחות". בסס את ההסבר אך ורק על חדשות אמת שמצאת; אל תמציא סיבות פיננסיות (כמו "העלאת ריבית", "דוחות מאכזבים") שלא אימתת. אם כל השוק זז יחד — הסבר את הסיבה המערכתית/הגאופוליטית האמיתית, לא סיבה ספציפית לחברה.
...
כתוב בעברית פשוטה בלבד — בלי אנגלית, בלי קישורים/מקורות, בלי markdown, בלי כותרות. עשיר ומעמיק (250-425 תווים), בלי לחזור על אותו מידע ובלי אמירות טריוויאליות שכל אחד יכול לנחש.
...
הסבר: <ההסבר בעברית, 2-3 משפטים מהותיים>`
  ...
  // Cross-engine fact-check to catch hallucinations before it reaches the user.
  explanation = await verifyInsight(explanation, `הנייר "${nameHe}" (${symbol})`, keys, provider)
